...
const TalentSignUp = ({ navigation }) => {
    ...
    const verifyEmail = async () => {
        setLoading(true);
        try {
            const response = await axios.post(`http://localhost:3001/api/verify`, { email, code: verificationToken });
    ...
    const handleSubmit = async () => {
        if (resumeData) {
            ...
            try {
                const response = await axios.post(`http://localhost:3001/api/signup/${roleUrlSegment}`, formData);
