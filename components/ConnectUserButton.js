...
const ConnectUserButton = ({ targetUserId }) => {
  ...
  useEffect(() => {
    const checkConnection = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:3001/api/user/check-connection/${targetUserId}`, {
  ...
  const handleSubmit = async (connectionData) => {
    ...
    try {
      const token = await AsyncStorage.getItem('auth_token');
      await axios.put('http://localhost:3001/api/user-connect', requestData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeleteConnection = async () => {
    try {
      const token = await AsyncStorage.getItem('auth_token');
      await axios.delete(`http://localhost:3001/api/delete-connection/${targetUserId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
